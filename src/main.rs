//! PDF Image Resampler (pure lopdf approach)
//!
...
impl<'a> ContentScanner<'a> {
                    ...
                    for (_, value) in pat_dict.iter() {
                        ...
                        if let Object::Reference(pat_id) = value {
                            ...
                            if let Ok(Object::Stream(stream)) = self.doc.get_object(*pat_id) {
                                let pattern_type = stream.dict.get(b"PatternType").ok().and_then(|p| match p {
                ...
                "gs" => {
                    ...
                    if i >= 1 {
                        ...
                        if let Some(&gs_id) = extgstates.get(name) {
                            let current_matrix = matrix_stack.last().copied().unwrap_or(Matrix::identity());
    ...
    fn scan_all_pages(&mut self) {
        ...
        for (page_num, &page_id) in pages.iter() {
            if self.verbose {
                println!("[Scanner] Scanning page {}...", page_num);
            }
...
fn decode_smask_stream(stream: &Stream, width: u32, height: u32) -> Result<Vec<u8>> {
    ...
    let decoded_data = match filter.as_deref() {
        ...
        None => content.clone(),
        Some(other) => {
            anyhow::bail!("Unsupported SMask filter: {}", other);
        }
    };
...
fn decode_image_stream(
    ...
    let decoded_data = match filter.as_deref() {
        ...
        Some("JPXDecode") => {
            ...
            let img =
                image::load_from_memory(content).context("Failed to decode JPEG2000 image")?;
            return Ok(img);
        }
        None => content.clone(),
        Some(other) => {
            anyhow::bail!("Unsupported filter: {}", other);
        }
    };
    ...
    match color_space {
        ...
        "DeviceGray" | "Gray" => {
            ...
            if bits_per_component == 8 && decoded_data.len() >= expected_size {
                let img =
                    image::GrayImage::from_raw(width, height, decoded_data[..expected_size].to_vec())
                        .context("Failed to create grayscale image from raw data")?;
        ...
        "DeviceCMYK" | "CMYK" => {
            ...
            if bits_per_component == 8 && decoded_data.len() >= expected_size {
                ...
                let img = RgbImage::from_raw(width, height, rgb_data)
                    .context("Failed to create RGB image from CMYK data")?;
                Ok(DynamicImage::ImageRgb8(img))
            } else {
                anyhow::bail!(
        ...
        "ICCBased" => {
            ...
            if decoded_data.len() >= pixels * 3 && bits_per_component == 8 {
                let img = RgbImage::from_raw(width, height, decoded_data[..pixels * 3].to_vec())
                    .context("Failed to create RGB image from ICCBased data")?;
                Ok(DynamicImage::ImageRgb8(img))
                ...
                let img = image::GrayImage::from_raw(width, height, decoded_data[..pixels].to_vec())
                    .context("Failed to create grayscale image from ICCBased data")?;
                Ok(DynamicImage::ImageLuma8(img))
            } else {
                anyhow::bail!("Could not determine ICCBased color space format");
            }
        }
        _ => {
            anyhow::bail!("Unsupported color space: {}", color_space);
        }
    }
}

/// Encode an image as JPEG and create a PDF stream
fn encode_as_jpeg_stream(img: &DynamicImage, quality: u8) -> Result<(Stream, u32, u32)> {
    let rgb = img.to_rgb8();
...
fn create_smask_stream(alpha_data: &[u8], width: u32, height: u32, quality: u8) -> Result<Stream> {
    let mut jpeg_bytes = Vec::new();
    ...
    encoder
        ...
        .context("Failed to encode SMask as JPEG")?;
...
fn encode_with_alpha_stream(
    img: &DynamicImage,
    quality: u8,
) -> Result<(Stream, Option<Stream>, u32, u32)> {
    ...
    std::io::Write::write_all(&mut encoder, &rgb_data).context("Failed to compress RGB data")?;
    let compressed_rgb = encoder.finish().context("Failed to finish compression")?;
...
/// Process images in PDF using display info from content stream scanning
fn process_images(
    input_path: &PathBuf,
    output_path: &PathBuf,
    ...
    if verbose {
        println!("[Process] Found {} image XObjects", image_objects.len());
    }
    ...
    for object_id in image_objects {
        ...
        if width == 0 || height == 0 {
            if verbose {
                println!("[Process] Skipping {:?}: invalid dimensions", object_id);
            }
            skipped_images += 1;
            continue;
        }
        ...
        let needs_resampling = current_dpi > target_dpi + 1.0 && current_dpi > min_dpi;

        // Calculate target dimensions
        let (target_width, target_height) = if needs_resampling {
            display_info.target_pixels_for_dpi(target_dpi)
        } else {
            (width, height)
        };
        ...
        if !needs_resampling && is_already_jpeg {
            if verbose {
                println!("  Skipping: Already JPEG at target DPI");
            }
            skipped_images += 1;
            continue;
        }
        ...
        if needs_resampling && target_width >= width && target_height >= height {
            if verbose {
                println!("  Skipping: Target dimensions not smaller");
            }
            skipped_images += 1;
            continue;
        }
        ...
        let mut img =
            match decode_image_stream(&stream, width, height, &color_space, bits_per_component) {
                ...
                Err(e) => {
                    if verbose {
                        println!("  Skipping: Could not decode: {}", e);
                    }
                    skipped_images += 1;
                    continue;
                }
                ...
                match decode_smask_stream(smask_stream, width, height) {
                    Ok(alpha_data) => {
                        ...
                        if let Some(rgba_img) = image::RgbaImage::from_raw(width, height, rgba_data)
                        {
                            ...
                            if verbose {
                                println!("    Decoded SMask alpha channel");
                            }
                        }
                    }
                    Err(e) => {
                        if verbose {
                            println!("    Warning: Could not decode SMask: {}", e);
                        }
                    }
                }
            }
        ...
        let resampled = if needs_resampling {
            ...
            if verbose {
                println!("  Re-encoding as JPEG (no resize needed)");
            }
            img
        };
        ...
        if img_has_alpha {
            let (mut new_stream, smask_stream, _, _) = encode_with_alpha_stream(&resampled, quality)?;
            ...
            if let Some(smask) = smask_stream {
                ...
                if verbose {
                    println!("      Preserved alpha channel with SMask {:?}", smask_id);
                }
            }

            doc.objects.insert(object_id, Object::Stream(new_stream));
        } else {
            if verbose && smask_id.is_some() {
                println!("      Converting opaque image to JPEG");
            }
            let (new_stream, _, _) = encode_as_jpeg_stream(&resampled, quality)?;
            doc.objects.insert(object_id, Object::Stream(new_stream));
        }
    ...
    Ok((total_images, resampled_images, skipped_images))
}
...
fn main() -> Result<()> {
    ...
    let doc = Document::load(&args.input)
        .with_context(|| format!("Failed to load PDF: {:?}", args.input))?;

    let mut scanner = ContentScanner::new(&doc, args.verbose);
    ...
    if args.verbose {
        println!(
            "\nFound display info for {} images",
            display_info_map.len()
        );
        for (id, info) in &display_info_map {
            println!(
                "  {:?}: {}x{} px @ {:.1}x{:.1} pt = {:.1} DPI",
                id,
                info.pixel_width,
    ...
    drop(doc); // Free memory before processing
    ...
    let (total_images, resampled_images, skipped_images) = process_images(
        &args.input,
        &args.output,
        &display_info_map,
        args.dpi,
    ...
    println!(
        ...
        total_images, resampled_images, skipped_images
    );
